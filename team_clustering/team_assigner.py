...
class TeamAssigner:
    ...
    def assign_team_colors(self,frame,detetcions):

        player_colors=[]
        for _,player in detetcions.items():
             bbox=player['bbox']
             player_color=self.get_player_color(frame,bbox)
             player_colors.append(player_color)

        kmeans=kmeans=KMeans(n_clusters=2, init="k-means++",n_init=1)
    ...
    def get_player_color(self,frame,bbox):
        ...
        kmeans=self.get_clustering(top_image)

        #Get clusters label
        kmeans_label=kmeans.labels_

        #reshape the label to image shape
        clustered_image=kmeans_label.reshape(top_image.shape[0],top_image.shape[1])

        #get the player clsuster
